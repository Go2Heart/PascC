import sys
sys.path.append('E:\Codes\PyCharmProjects\PascC')
import logging
from src.lexical_syntactic.plexer import Lexer

def log(lexer, output_file=None):
    while True:
        token = lexer.token()
        if not  token:
            break
    ...
    if lexer.errorFlag:
        logging.error("Error ocurrs in lexer")
        for item in lexer.errormes:
            print(item)

def test_0():
    """语法分析前序测试"""
    test_lexer = Lexer()
    test_lexer.load_file('./test_semantic/test_var_refer/test_var_refer_1.pas')
    test_lexer.scan()
def test_lexer():
    """综合正确性测试 quicksort"""
    test_lexer = Lexer()
